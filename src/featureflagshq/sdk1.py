"""
FeatureFlagsHQ SDK Main Class - Fixed Version
...
class FeatureFlagsHQSDK:
    ...
    def __init__(
        ...
        if not offline_mode:  # Only setup session if not offline
        ...
        try:
            if not offline_mode:
                try:
                    ...
                    logger.warning(f"Initial fetch failed, continuing in degraded mode: {e}")
                    # Still start background tasks for recovery
                    try:
                        self._start_background_tasks()
                    except Exception as bg_e:
                        logger.warning(f"Background tasks startup failed: {bg_e}")
            else:
                logger.info("FeatureFlagsHQ: SDK initialized in offline mode")
        finally:
            # Always mark initialization as complete, even if initialization fails
            # This prevents the get() method from blocking indefinitely
    ...
    def _rate_limit_check(self, user_id: str) -> bool:
        ...
        self._rate_limits = {
            uid: (request_count, last_time) for uid, (request_count, last_time) in self._rate_limits.items()
    ...
    def _fetch_flags(self) -> Dict[str, FeatureFlagsHQFlag]:
        """Fetch feature flags from FeatureFlagsHQ API with timeout protection"""
        if self.offline_mode:
            logger.debug("FeatureFlagsHQ: Offline mode, skipping flag fetch")
            return {}
        ...
        try:
            ...
            # Use a shorter timeout for the actual request to prevent hanging
            response = self.session.get(
                flags_url,
                headers=headers,
                timeout=min(self.timeout, 10)  # Cap at 10 seconds to prevent hanging
            )
            ...
            logger.warning("FeatureFlagsHQ: Request timeout during flag fetch")
            return {}
        except requests.exceptions.ConnectionError:
            self._record_api_failure('network_errors')
            logger.warning("FeatureFlagsHQ: Connection error during flag fetch")
            return {}
            ...
            logger.error(f"FeatureFlagsHQ: Unexpected error during flag fetch: {e}")
            return {}
    ...
    def get(
        ...
        """Secure version of get method with enhanced validation and offline support"""

        # Wait for initialization if still in progress (with timeout)
        if not self._initialization_complete.wait(timeout=5):
            logger.warning("FeatureFlagsHQ: Initialization still in progress, proceeding anyway")
        ...
        with measure_time() as get_total_time:
            ...
            try:
                ...
                logger.error(f"Error accessing flags: {e}")
                flag = None
            ...
            try:
                ...
                evaluation_context['offline_mode'] = self.offline_mode
    ...
    def get_user_flags(self, user_id: str, segments: Optional[Dict[str, Any]] = None,
        ...
        for flag_key, flag in flags_to_evaluate.items():
            try:
                ...
                try:
                    if hasattr(flag, '_get_typed_default_value'):
                        user_flags[flag_key] = flag._get_typed_default_value()
                    else:
                        # Fallback based on flag type
                        if hasattr(flag, 'type'):
                            if flag.type == 'boolean':
                                user_flags[flag_key] = False
                            elif flag.type == 'string':
                                user_flags[flag_key] = ""
                            elif flag.type in ['integer', 'number']:
                                user_flags[flag_key] = 0
                            elif flag.type == 'json':
                                user_flags[flag_key] = {}
                            else:
                                user_flags[flag_key] = None
                        else:
                            user_flags[flag_key] = None
                except:
                    user_flags[flag_key] = None
            ...
            with self._flags_lock:
                return {
                    flag_key: {
                        ...
                        'segments_count': len(flag.segments) if hasattr(flag, 'segments') and flag.segments else 0,
                        'rollout_percentage': flag.rollout.percentage if hasattr(flag,
                                                                                 'rollout') and flag.rollout and hasattr(
                            flag.rollout, 'percentage') else 0,
                        'version': flag.version if hasattr(flag, 'version') else 'unknown',
                        'updated_at': flag.updated_at if hasattr(flag, 'updated_at') else 'unknown',
                        'provider': BRAND_NAME
                    }
    ...
    def get_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                system_info = {
                    'sdk_version': self.system_metadata.sdk_version if hasattr(self.system_metadata,
                                                                               'sdk_version') else __version__,
                    'python_version': self.system_metadata.python_version if hasattr(self.system_metadata,
                                                                                     'python_version') else 'unknown',
                    'platform': self.system_metadata.platform if hasattr(self.system_metadata,
                                                                         'platform') else 'unknown'
                }
            except Exception:
                system_info = {'sdk_version': __version__, 'python_version': 'unknown', 'platform': 'unknown'}
    ...
    def shutdown(self):
        ...
        for thread, name in threads_to_join:
            if thread and thread.is_alive():
                ...
                thread.join(timeout=5)  # Reduced timeout to prevent hanging
                if thread.is_alive():
    ...
    def _initial_fetch(self):
        """Initial fetch of feature flags with timeout protection"""
        try:
            new_flags = self._fetch_flags()
            with self._flags_lock:
                self.feature_flags = new_flags
            self.stats['last_sync'] = datetime.now(timezone.utc).isoformat()
            logger.info("FeatureFlagsHQ: Initial fetch completed")

        except Exception as e:
            logger.warning(f"FeatureFlagsHQ: Initial fetch failed: {e}, continuing in degraded mode")
    ...
    def _upload_user_logs(self):
        ...
        if self.offline_mode or self.user_access_logs.empty() or not self._check_circuit_breaker():
            return
        ...
        try:
            ...
            response = self.session.post(
                ...
                json=payload,
                headers=headers,
                timeout=min(self.timeout, 10)  # Cap timeout
            )
            response.raise_for_status()
            self._record_api_success()
            ...
            # Put logs back for retry (but limit to prevent memory bloat)
            if len(logs_batch) <= 10:  # Only retry small batches
